...
export default function RetailerHomeScreen(props: RetailerHomeScreenProps) {
    ...
    useEffect(() => {
        ...
        async function getProducts(){
            setLoading(true);
            const products: any = await getDataByQuery("products", "retailerId", "==", user.id);
            ...
            const paginatedProducts = paginateArray(products, 5);
            setEntity(products);
    ...
    useEffect(() => {
        ...
        setDisplayEntity(paginateArray(filteredData, 5));
        ...
        <GestureHandlerRootView style={{height: height, backgroundColor: mainStyles.mainColors.background}}>
            ...
            <ScrollView>
                <View style={styles.container}>
                    ...
                    <Text style={{textAlign: 'center', fontWeight: 'bold', marginBottom: 10, fontSize: 18}}>MEUS PRODUTOS</Text>
                    {loading ? (<Loader />) : (
                        <Fragment>                            
                            <View style={styles.productsContainer}>
                                {displayEntity && displayEntity[currentPage].map((product: any) => (
                                    <ProductRow key={product.id} product={product} setLoading={setLoading} setReload={setReload}/>
                                ))}
                            </View>
                            
                            {!displayEntity && <Text style={{textAlign: 'center', marginBottom: 20}}>Nenhum produto foi encontrado...</Text>}
                            ...
                            <View style={{marginBottom: 40, justifyContent: 'center', flexDirection: 'row'}}>
                                <TouchableOpacity style={styles.RegisterProductBtn} onPress={() => router.push("RegisterProduct/RegisterProductScreen")}>
                                    <Icon name='inventory-2' color='white'/>
                                    <Text style={{fontWeight: 'bold', color: 'white'}}>CADASTRAR PRODUTO</Text>
                                </TouchableOpacity>
                            </View>
